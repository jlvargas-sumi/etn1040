<h4 id="titulo-principal">
    NOTAS DE ESTUDIANTES INSCRITOS (DOCENCIA)
</h4>
...
<table class="sin-borde">
    <tbody>
        <tr>
            ...
            <td class="nombre-celda">Docente:</td>
            <td>{{ $docente->persona_primer_apellido }} {{ $docente->persona_segundo_apellido }} {{ $docente->persona_nombres }}</td>
        ...
        <tr>
            ...
            @foreach ($aperturas as $asignatura)
                ...
                @foreach ($asignatura->docencia_ponderacion['ponderacionesPrincipal'] as $i => $ponderacion)
                    <th>{{ $campo }}{{ $i }}</th>
                @endforeach
                <th class="grey darken-1 centro white-text">Total {{ $campo }}</th>
                @foreach ($asignatura->docencia_ponderacion['ponderacionesSecundaria'] as $i => $ponderacion)
                    <th>A{{ $i }}</th>
                @endforeach
                <th class="grey darken-1 centro white-text">Total A</th>
                <th class="grey darken-3 centro white-text">Total</th>
            @endforeach
        </tr>
    </thead>
    <tbody>
