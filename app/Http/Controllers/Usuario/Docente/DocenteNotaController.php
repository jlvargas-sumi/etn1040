...
class DocenteNotaController extends Controller
{
    ...
    public function indice($aperturaId, $periodo = null, $gestion = null)
    {
        ...
        return view('usuario.docente.notas',
            compact('asignatura', 'aperturas', 'docente', 'inscritosTeoria', 'inscritosLaboratorio', 'aperturaId', 'periodo', 'gestion')
        );
    }
    public function registrar(Request $request)
    {
        ...
        if ($registrar) {
            ...
            return to_route('docente.notas', [$aperturaId, $periodo, $gestion]);
        }
        $this->actividades->registrarActividad(
            Auth::User()->usuario_id,
            $modulo = 'Docentes - Notas',
    ...
    public function exportarPdf(Request $request, $aperturaId, $periodo = null, $gestion = null)
    {
        ...
        $verificarMiAperturaId = $this->asignaturas->verificarMiAperturaIdDocente($aperturaId, $this->personaId, $periodo, $gestion);
        if (!$verificarMiAperturaId) {
            return to_route('docente.notas', [$periodo, $gestion]);
        }
        ...
        $inscritosLaboratorio = $this->asignaturas->estudiantesInscritosMateria($aperturaIdLaboratorio);
        try {
            $vista =  View::make('usuario.docente.notas-pdf', compact(
                'asignatura', 'aperturas', 'docente', 'inscritosTeoria', 'inscritosLaboratorio',
            ...
            return $pdf->stream('notasDocPDF.pdf', ['Attachment' => false]);
        } catch (\Throwable $th) {
            $this->actividades->registrarActividad(
                Auth::User()->usuario_id,
                $modulo = 'Docentes - Notas',
