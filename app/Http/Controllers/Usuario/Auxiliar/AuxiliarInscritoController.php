...
class AuxiliarInscritoController extends Controller
{
    private $actividades;
    private $plantel;
    private $asignaturas;
    ...
    public function indice($aperturaId, $periodo = null, $gestion = null)
    {
        ...
        return view('usuario.auxiliar.inscritos', 
            compact('asignatura', 'auxiliar', 'inscritos', 'aperturaId', 'periodo', 'gestion')
        );
    }
    public function exportarPdf(Request $request, $aperturaId, $periodo = null, $gestion = null)
    {
        ...
        try {
            $vista =  View::make('usuario.auxiliar.inscritos-pdf', compact(
                'asignatura', 'auxiliar', 'inscritos', 'aperturaId', 'periodo', 'gestion'
            ))->render();
            $pdf = PDF::loadHtml($vista, 'UTF-8')->setPaper('letter')->setWarnings(false);
            $this->actividades->registrarActividad(
                Auth::User()->usuario_id,
                $modulo = 'Auxiliares - Inscritos',
                $accion = 7,
                $resultado = 1,
